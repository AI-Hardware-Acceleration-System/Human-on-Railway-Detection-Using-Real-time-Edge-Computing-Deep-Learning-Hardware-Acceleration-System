...
>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;在平均池化的硬體設計中，上列的資料要等到下列資料進來時才可運算，這也因此需要很多暫存器。而本專題的設計理念為平均池化採兩列一組處理的運行方式，上列每進來兩個值便會將其加起來並存到暫存器內，到下列時同樣每兩個值便會將結果加起來，但同時也會加上上列對應的加起來的值以及乘上0.25即得到一個輸出結果，示意圖如下圖一。<br/>
>圖一:<br/>
